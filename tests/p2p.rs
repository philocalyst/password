...
//!
//! Spins up two in-process IrohSync nodes and verifies payload sync across peers
//! via tickets, rather than just unit-testing serialization.

use std::time::Duration;
use tokio::time::timeout;

use password::{
    PasswordStore, AccountName, Item,
    models::{OnlineAccount, AccountStatus},
    p2p::{IrohSyncHandle, encode_store, decode_store},
};
...
async fn sync_functions_between_concurrent_peers() {
    ...
    let receiver_task = tokio::spawn(async move {
        ...
        ).await.expect("receive timed out").expect("receive payload");
    ...
    assert_eq!(
        received.items.len(),
        original.items.len(),
        "item count across sync must match"
    );
