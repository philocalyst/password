...
/// Every method takes a `branch` argument (string) which maps to a Pijul channel
/// for multi-party isolation.
...
/// Every operation takes a `branch` to scope history to a specific party/channel.
pub trait Versioned {
    type Error: std::error::Error + Send + Sync + 'static;
    ...
    /// Revert a single entry in a branch to the state it had
    /// immediately after `target` was applied.
    fn revert_entry(
        &self,
        branch: &str,
        name: &AccountName,
    ...
    /// Chronological change log, optionally filtered to a single entry
    /// in a branch.
    fn log(
        &self,
        branch: &str,
    ...
    /// Structured diff of a single entry between `from` and `to`
    /// inside a branch.
    fn diff_entry(
        &self,
        branch: &str,
    ...
    /// Return the content of one entry as it existed immediately after `at`
    /// was applied.
    fn show_entry_at(
        &self,
        branch: &str,
